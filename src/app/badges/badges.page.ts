...
export class BadgesPage implements OnInit {
  ...
  ngOnInit() {
    this.badgeUserService.getBadgeUserById('RDdHIsiP5ChIUiipTEYPyL4wuuH2').subscribe((badges_users) => {
      this.badges_users = <any[]>badges_users;
      console.log(badges_users);
    });

    this.badges_users.forEach(element => {
      this.badges.push(this.badgeService.getBadgesById(element['badge']))
